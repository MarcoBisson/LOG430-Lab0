...
export class ProductService {
     ...
     * @returns Une liste de produits correspondant au terme de recherche.
     */
    async searchProducts(searchTerm: string, page?: number, limit?: number): Promise<{ products: Product[]; total: number }> {
        const { products, total } = await this.productRepo.searchProducts(searchTerm, page, limit);
        return {
            products: products.map(
                p => new Product(p.id, p.name, p.price, p.description, p.category),
            ),
